alter table public.heyreach_conversations
    add column auto_tag text;

alter table public.heyreach_conversations
    drop constraint if exists heyreach_conversations_reply_type_check;

alter table public.heyreach_conversations
    add constraint heyreach_conversations_reply_type_check check (
        reply_type in ('positive', 'ooo', 'negative')
    );

create index if not exists heyreach_conversations_reply_type_lead_id_idx
    on public.heyreach_conversations (reply_type, lead_id);

alter table public.heyreach_import_runs
    add column reply_types text[] not null default array['positive']::text[];

alter table public.heyreach_import_runs
    add constraint heyreach_import_runs_reply_types_check check (
...
begin
    ...
    values (
        ...
        nullif(p_conversation ->> 'auto_tag', ''),
    ...
    set
        ...
        auto_tag = coalesce(
            excluded.auto_tag,
            public.heyreach_conversations.auto_tag
        ),
