...
class TableRepository:
    ...
    async def insert_sheriff_run(self, record: dict[str, Any]) -> dict[str, Any]:
        response = await self._db.table("table_sheriff_runs").insert(record).execute()
        return response.data[0]

    async def update_sheriff_run(
        self, run_id: str, values: dict[str, Any]
    ) -> dict[str, Any] | None:
        payload = {"updated_at": to_iso(utc_now()), **values}
        response = await (
            self._db.table("table_sheriff_runs")
            .update(payload)
            .eq("id", run_id)
            .execute()
        )
    ...
    async def get_sheriff_run_by_id(self, run_id: str) -> dict[str, Any] | None:
        response = await (
            self._db.table("table_sheriff_runs")
            .select("*")
            .eq("id", run_id)
    ...
    async def insert_sheriff_run_items(
        self, items: list[dict[str, Any]]
    ) -> list[dict[str, Any]]:
        if not items:
            return []
        response = await (
            self._db.table("table_sheriff_run_items").insert(items).execute()
        )
        return response.data

    async def list_sheriff_run_items(self, run_id: str) -> list[dict[str, Any]]:
        response = await (
            self._db.table("table_sheriff_run_items")
            .select("*")
            .eq("run_id", run_id)
            .order("created_at")
            .order("id")
    ...
    async def update_sheriff_run_item(
    ...
    async def get_email_enrichment_run_by_id(
        self, run_id: str
    ) -> dict[str, Any] | None:
        response = await (
            self._db.table("table_email_enrichment_runs")
    ...
    async def insert_email_enrichment_run_items(
        self, items: list[dict[str, Any]]
    ) -> list[dict[str, Any]]:
        if not items:
            return []
        response = await (
            self._db.table("table_email_enrichment_run_items").insert(items).execute()
        )
        return response.data

    async def list_email_enrichment_run_items(
        self, run_id: str
    ) -> list[dict[str, Any]]:
        response = await (
